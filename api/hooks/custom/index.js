        ...
        '/*': {
          ...
          fn: async function(req, res, next){
            ...
            if (loggedInUser.lastSeenAt < now - MS_TO_BUFFER) {
              ExpanceUser.updateOne({id: loggedInUser.id})
              .set({ lastSeenAt: now })
              .exec((err)=>{
                if (err) {
            ...
            if (req.method === 'GET') {
              ...
              if (sanitizedUser.password) {
                sails.log.warn('The logged in user record has a `password` property, but it was still there after pruning off all properties that match `protect: true` attributes in the ExpanceUser model.  So, just to be safe, removing the `password` property anyway...');
