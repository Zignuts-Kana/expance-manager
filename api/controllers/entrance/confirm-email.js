...
(for existing users who just changed their email address).`,
  ...
  fn: async function ({token}) {
    ...
    var user = await ExpanceUser.findOne({ emailProofToken: token });
    ...
    if (user.emailStatus === 'unconfirmed') {
      ...
      await ExpanceUser.updateOne({ id: user.id }).set({
      ...
      if (await ExpanceUser.count({ emailAddress: user.emailChangeCandidate }) > 0) {
      ...
      await ExpanceUser.updateOne({ id: user.id })
      .set({
        emailStatus: 'confirmed',
        emailProofToken: '',
        emailProofTokenExpiresAt: 0,
      ...
      throw new Error(`Consistency violation: ExpanceUser ${user.id} has an email proof token, but somehow also has an emailStatus of "${user.emailStatus}"!  (This should never happen.)`);
    }
